# jt-live-whisper - 即時英翻中字幕系統 v1.7.5 (by Jason Cheng) - 安裝與使用 SOP
...
即時轉錄的 ASR 辨識裝置固定使用 BlackHole 2ch（只擷取對方聲音），這樣辨識最準確。但如果你啟用了 `--record` 錄音功能，想要**同時錄下對方和自己的聲音**，就需要建立聚集裝置（Aggregate Device），讓錄音裝置可以選它。
...
建好之後，啟動程式時選擇錄製音訊，在錄音裝置選單選這個聚集裝置即可。程式會自動優先推薦聚集裝置作為錄音預設。

**ASR 辨識裝置 vs 錄音裝置的差別：**

| 用途 | 選擇的裝置 | 擷取內容 | 說明 |
|---|---|---|---|
| ASR 辨識 | BlackHole 2ch | 僅對方聲音 | 只送對方語音給 AI 辨識，避免自己的聲音干擾 |
| 錄音 | 聚集裝置 | 對方 + 自己 | 同時錄下雙方聲音，事後可回聽或離線再處理 |
...
選擇錄製後，會進入錄音裝置選擇。預設優先選聚集裝置（Aggregate Device，可同時錄到對方與自己的聲音），其次選 BlackHole（只錄系統音訊）。錄音裝置可以和 ASR 辨識裝置不同：ASR 只需要 BlackHole（對方聲音），但錄音可以選聚集裝置（雙方聲音）。

程式會在即時辨識的同時將音訊存為 16-bit PCM WAV 檔。錄音檔名含時間戳，例如 `recording_20260304_143000.wav`。停止時（Ctrl+C 或 Ctrl+S）會自動關閉錄音並顯示儲存路徑。錄音檔可用於事後回聽或以 `--input` 離線再處理。
...
搭配 `--summarize` 和 `--diarize`，可對匯入的錄音檔產生含說話者辨識的摘要與校正逐字稿：
...
`--diarize` 需搭配 `--input` 使用，不適用於即時模式。即時模式無法即時辨識說話者，因此建議在即時模式啟用錄音功能（`--record`），事後再將錄音檔以 `--input` + `--diarize` 匯入做說話者辨識：

```bash
# 步驟 1：即時模式啟用錄音
./start.sh --record

# 步驟 2：事後用錄音檔做說話者辨識 + 翻譯 + 摘要
...
- HTML 摘要不同說話者使用不同顏色（8 色循環），延續段落繼承同色
