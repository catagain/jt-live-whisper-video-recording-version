# jt-live-whisper - 即時英翻中字幕系統 v1.9.4 (by Jason Cheng) - 安裝與使用 SOP
...
- **單機模式** -- 一台 MacBook 即可完成所有處理。語音辨識（Whisper/Moonshine）、翻譯（LLM/Argos）全部在本機 CPU 執行，不需要額外硬體。適合個人使用、外出攜帶。

- **本機 + GPU 伺服器模式** -- MacBook 負責音訊擷取與介面操作，語音辨識和講者辨識交由區域網路內的 GPU 伺服器（如 DGX Spark、Ubuntu + NVIDIA GPU）處理。離線辨識速度快 5-10 倍，仍然是全地端架構，資料僅在區域網路內傳輸。適合需要處理大量音訊或追求即時辨識品質的場景。
...
| `--llm-host HOST` | LLM 伺服器位址，自動偵測 Ollama 或 OpenAI 相容 (支援 host:port 格式) | 192.168.1.40:11434 |
...
**注意：** Ctrl+S 摘要是以即時轉譯過程中產生的文字記錄做摘要，不會重新處理錄音檔，也不包含講者辨識。即時模式僅處理系統音訊（對方聲音），無法涵蓋你自己的發言。如需包含雙方聲音的完整逐字稿、講者辨識與摘要，請啟用錄音功能，事後用 `--input` 匯入錄音檔做完整處理：
...
| `config.json` | 使用者設定檔（自動產生，含 LLM 伺服器位址、遠端 GPU 設定等） |
...
**改進**
- 支援多實例同時使用遠端 GPU 伺服器：啟動時先檢查伺服器是否已在運行，是則直接沿用；退出時不再關閉伺服器
...
- Ctrl+S 摘要說明新增注意事項：此摘要基於即時轉譯文字記錄，不含講者辨識，如需完整功能請用 `--input` 離線處理
