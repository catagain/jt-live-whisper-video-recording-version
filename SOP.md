...
即時英翻中字幕系統 v2.11.2 (by Jason Cheng)
...
| `--input FILE [...]` | 離線處理音訊檔（用 faster-whisper 辨識）。不帶 `--mode` 時進入互動選單。指定兩個雙向配對檔案時自動偵測並合併處理 | |
...
- **`en_zh`（英中雙向）**：對方英文翻中文 + 自己中文翻英文
- **`ja_zh`（日中雙向）**：對方日文翻中文 + 自己中文翻日文
...
**CUDA 版本注意事項：** faster-whisper 使用的 CTranslate2 引擎需要 CUDA 12.x 的程式庫（`cublas64_12.dll`）。若系統安裝的是 CUDA Toolkit 13.x，會出現「Library cublas64_12.dll is not found or cannot be loaded」錯誤。解決方式：另外安裝 [CUDA Toolkit 12.8](https://developer.nvidia.com/cuda-12-8-0-download-archive)（可與 13.x 並存），安裝腳本會自動偵測並提醒。
