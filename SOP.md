...
程式會在即時辨識的同時將音訊存為 16-bit PCM WAV 檔。錄音從開始到停止全程錄在同一個檔案，不會自動切檔。錄音檔名含時間戳，例如 `recording_20260304_143000.wav`。停止時（Ctrl+C）自動關閉錄音並顯示儲存路徑。
...
| `en2zh_summary_*.txt` / `zh2en_summary_*.txt` / `en_summary_*.txt` / `zh_summary_*.txt` | 摘要檔（--summarize 產生） |
| `SOP.md` | 本文件 |
